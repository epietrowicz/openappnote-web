...
function getModelExtension (url, format) {
...
const ModelMesh = ({
  ...
  useFrame((_, dt) => {
    if (!outer.current || !content) return
...
const GlbModelInner = (props) => {
  const { scene } = useGLTF(props.url)
  const content = useMemo(() => scene.clone(), [scene])
  return <ModelMesh content={content} {...props} />
}

const FbxModelInner = (props) => {
  const fbx = useFBX(props.url)
  const content = useMemo(() => fbx.clone(), [fbx])
  return <ModelMesh content={content} {...props} />
}

const ObjModelInner = (props) => {
  const obj = useLoader(OBJLoader, props.url)
  const content = useMemo(() => obj.clone(), [obj])
  return <ModelMesh content={content} {...props} />
}

const ModelInner = (props) => {
  const ext = getModelExtension(props.url, props.format)
  if (ext === 'glb' || ext === 'gltf') return <GlbModelInner {...props} />
  if (ext === 'fbx') return <FbxModelInner {...props} />
  if (ext === 'obj') return <ObjModelInner {...props} />
