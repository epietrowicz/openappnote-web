...
async function getFrontier () {
  const state = await getCrawlState(FRONTIER_STATE_ID)
...
async function crawlOneRangePage (range, deadline) {
  const id = rangeId(range)
  const state = await getCrawlState(id)
  ...
  for (const { owner, repo } of uniqueRepos.values()) {
    ...
    if (repository) await crawlRepository(repository, deadline)
  }

  const exhausted = items.length === 0 || page * CODE_RESULTS_PER_PAGE >= Math.min(totalCount, SEARCH_RESULT_CAP)
  await setCrawlState({ id, kind: 'code-search-range', query, lastPage: page, totalHits: totalCount, exhausted })

  return { done: exhausted }
}

export async function runChannelACrawl ({ budgetMs = 270_000 } = {}) {
  const startedAt = Date.now()
  const deadline = startedAt + budgetMs

  let frontier = await getFrontier()
  let queriesProcessed = 0

  while (frontier.length > 0 && Date.now() < deadline) {
    const range = frontier[0]
    const result = await crawlOneRangePage(range, deadline)
    queriesProcessed += 1
