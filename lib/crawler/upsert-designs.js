...
import { filterProjectFiles } from '@/lib/github-tree'
...
export async function buildProjectDocument (repository, projectDir, tree) {
  const owner = repository.owner.login
  const repo = repository.name

  const projectFiles = filterProjectFiles(tree, projectDir)
  ...
  return {
    ...
    // Cheap completeness signals used to derank schematic-only stubs in search
    // (see rankingRules in meilisearch-schema.js) - not exposed in the UI.
    hasPcb: pcbFiles.length > 0,
...
export async function upsertDesignDocuments (documents, client) {
  if (documents.length === 0) return
  const index = getDesignsIndex(client)
